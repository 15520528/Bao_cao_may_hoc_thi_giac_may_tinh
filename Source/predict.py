...
clf = joblib.load('modelSVM.joblib')
...
def buttonClick():
    ...
    root.filename = filedialog.askopenfilename(initialdir="./imageTest", title="Select file",
    ...
    #luu anh
    files = glob.glob('pridict/image/*')
    for f in files:
        os.remove(f)
    files = glob.glob('pridict/feature/*')
    for f in files:
        os.remove(f)
    sampleNum = 0   
    for(x,y,w,h) in faces:
        sampleNum = sampleNum + 1
        cv2.imwrite("pridict/image/img." + '1.' + str(sampleNum) + ".jpg", im[y:y + h, x:x + w])
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.putText(im, results[i], (x, y + h), font, 2, fontcolor)
        i = i+1
    plt.imshow(im)
    plt.show()
